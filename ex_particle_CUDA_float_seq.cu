...
__device__ double calcLikelihoodSum(unsigned char * I, int * ind, int numOnes, int index) {
    ...
    int x;
    /**
     * Apparently, the likelihood function has something 
     * to do with the intensity of the fore/background. 
    */
    for (x = 0; x < numOnes; x++)
...
__global__ void find_index_kernel(double * arrayX, double * arrayY, double * CDF, double * u, double * xj, double * yj, double * weights, int Nparticles) {
    ...
    if (i < Nparticles) {
        ...
        /**
         * This will surely create a lot of thread divergence. 
         * TODO: Investigate on how to optimize this. 
        */
        for (x = 0; x < Nparticles; x++) {
            if (CDF[x] >= u[i]) {
                index = x;
        ...
        /**
         * Set to the last one if not found. 
        */
        if (index == -1) {
            index = Nparticles - 1;
        }
        ...
        /**
         * Selectively set the parents of the next
    ...
    /**
     * Do we actually need synchornization? 
     * TODO: Investigate this synchronization. 
    */
    __syncthreads();
}
...
__global__ void normalize_weights_kernel(double * weights, int Nparticles, double* partial_sums, double * CDF, double * u, int * seed) {
    ...
    /**
     * Avoid access to the global memory by 
     * having the first thread in a block pre-fetch 
     * the value into the shared memory. 
    */
    if(0 == threadIdx.x)
        sumWeights = partial_sums[0];
    
    /**
     * Indeed, other threads must wait for the 
     * sum to be copied before using it. 
    */
    __syncthreads();
    
    /**
     * Simply normalize the weight, such that 
     * the weight sum to one. 
    */
    if (i < Nparticles) {
    ...
    /**
     * Wait for all weight to be normalized
     * before further calculations can be done. 
    */
    __syncthreads(); 
    
    /** 
     * Have first thread in the block calculate 
     * the cumulative distribution function. 
     * TODO: Optimize this by coalescing/reduction
    */
    if (i == 0) {
    ...
    /**
     * Begin systematic resampling by 
     * only allow the first thread to sample
     * from uniform random. All other threads 
     * follows the sampling on its own strata. 
    */
    __syncthreads();
    
    /**
     * First thread in the block copy the 
     * sampling result onto a shared mem
    */
    if(0 == threadIdx.x) 
        u1 = u[0];
    ...
    /**
     * Other threads read from the shared
     * memory instead shift the result
     */
    if (i < Nparticles) {
...
/**
 * Reduction operation. Combine all sum from 
 * all blocks before normalizing the weights
*/
 ...
 * param12: Nfr
 *****************************/
__global__ void likelihood_kernel(double * arrayX, double * arrayY, double * xj, double * yj, double * CDF, int * ind, int * objxy, double * likelihood, unsigned char * I, double * weights, int Nparticles, int countOnes, int max_size, int k, int IszY, int Nfr, int *seed, double* partial_sums) {
    int block_id = blockIdx.x;
    int i = blockDim.x * block_id + threadIdx.x;
    int y;
    ...
    /**
     * This block should be relatively optimized. 
     * Unless we instead use cuRAND.
     * TODO: Try cuRAND for sampling, maybe faster. 
    */
    if (i < Nparticles) {
        arrayX[i] = xj[i]; 
        arrayY[i] = yj[i]; 
        ...
        /**
         * Reset weight after systematic 
         * resamplings, huh.
        */
        weights[i] = 1 / ((double) (Nparticles));

        /** 
         * The proposal distribution. Sampled 
         * from a presumably normal distribution. 
         * Seems like we are sampling from the 
         * true transition probability, notice
         * the variance of the distribution. 
         * TODO: Particle Gibbs for parameter inference. 
         * This will give me a chance to understand particle gibs
         * and practice parameter inference for other tasks. 
        */
    ...
    */
    ...
    if (i < Nparticles) {
        ...
        /**
         * Given the shape of the object to be tracked. 
         * We calculate the likelihood of the observation
         * if the object is centered at the particle position. 
         * NOTE: I don't think there is a control divergence here. 
         * TODO: Factorizing this part out could lead to improvement!
        */
     ...
     * because not all threads will pass into 
     * the next condition. 
    */
    buffer[threadIdx.x] = 0.0;

    /**
     * Do we actually need to wait? 
     * TODO: Investigate this synchronization. 
    */
    __syncthreads();
    ...
    /**
     * Definitely need to wait because next operation 
     * is reduction of the shared memory, buffer. 
    */
    __syncthreads();

    /** 
     * Reduction operation with optimized 
     * control divergence (compared to 
     * even and odd indexed reduction)
    */
    ...
    /**
     * Put the partial sum of current block 
     * onto the global memory. 
    */
    if (threadIdx.x == 0) {
    ...
    /**
     * Do we actually need to compute this, given a default stream? 
     * TODO: Investigate this synchornization. 
    */
    __syncthreads();
}
...
void particleFilter(unsigned char * I, int IszX, int IszY, int Nfr, int * seed, int Nparticles) {
    ...
    FILE *wfp = fopen("weights.csv", "w"); 
    for (ii = 0; ii < Nparticles - 1; ii += 1)
        fprintf(wfp, "%d, ", ii); 
    fprintf(wfp, "%d\n", ii); 
    FILE *pfp = fopen("pred_path.txt", "w"); 
    fprintf(pfp, "PREDICTED PATH: \n"); 
#else
